...
std::string Camera::name() const { return "Camera"; }

// Camera::Camera(const glm::vec3 position, glm::vec3 up, float yaw, float
// pitch)
//     : m_position(position), m_up(up), m_yaw(yaw), m_pitch(pitch) {
//     m_worldUp = glm::normalize(up);
//
//     glm::vec3 front;
//     front.x = std::cos(glm::radians(m_yaw)) *
//     std::cos(glm::radians(m_pitch)); front.y =
//     std::sin(glm::radians(m_pitch)); front.z = std::sin(glm::radians(m_yaw))
